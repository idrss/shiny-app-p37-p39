...
ui <- bs4DashPage(
  ...
  header = bs4DashNavbar(title = "ANALYSE DE LA STRUCTURE FINANCIÈRE"),
  ...
  sidebar = bs4DashSidebar(
    ...
    bs4SidebarMenu(
      ...
      bs4SidebarMenuItem("Tableau complet", tabName = "tableaux", icon = icon("table")),
      ...
      bs4TabItem(tabName = "tableaux",
                 fluidRow(
                   ...
                   bs4Card(width = 9,
                           ...
                           plotlyOutput("plot_tableau", height = "600px"))
                 )
      ),
...
server <- function(input, output, session) {
  ...
  filtered_tableau <- reactive({
    req(creds$auth)
    df <- data_tableau %>% filter(Libelle %in% input$filter_libelle, Famille %in% input$filter_famille)
    return(df)
  })
  ...
  # --- Plot ---
  output$plot_tableau <- renderPlotly({
    df <- filtered_tableau()
    plot_ly(df, x = ~Famille, y = ~Q1, type = 'bar', color = ~Famille, colors = famille_colors) %>%
      add_trace(y = ~Med, name = 'Med') %>%
      add_trace(y = ~Q3, name = 'Q3') %>%
      layout(barmode = 'group', title = 'Indicateurs A1-D34 par Famille')
  })
