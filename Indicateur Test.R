    ...
    bs4TabItems(
      ...
      bs4TabItem(
        ...
        fluidRow(
          bs4Card(
            width = 3,
            pickerInput("filter_code_p37", "Choisir Code:", 
                        choices = unique(data_p37$Code),
                        selected = unique(data_p37$Code)[1],
                        multiple = TRUE),
            pickerInput("filter_type_p37", "Choisir Type:", 
                        choices = unique(data_p37$Type),
                        selected = unique(data_p37$Type)[1],
                        multiple = TRUE),
      ...
      bs4TabItem(
        ...
        fluidRow(
          bs4Card(
            width = 3,
            pickerInput("filter_code_p38", "Choisir Code:", 
                        choices = unique(data_p38$Code),
                        selected = unique(data_p38$Code)[1],
                        multiple = TRUE),
            pickerInput("filter_type_p38", "Choisir Type:", 
                        choices = unique(data_p38$Type),
                        selected = unique(data_p38$Type)[1],
                        multiple = TRUE),
      ...
      bs4TabItem(
        ...
        fluidRow(
          bs4Card(
            width = 3,
            pickerInput("filter_code_p39", "Choisir Code:", 
                        choices = unique(data_p39$Code),
                        selected = unique(data_p39$Code)[1],
                        multiple = TRUE),
            pickerInput("filter_type_p39", "Choisir Type:", 
                        choices = unique(data_p39$Type),
                        selected = unique(data_p39$Type)[1],
                        multiple = TRUE),
...
server <- function(input, output, session) {
  ...
  # ---- P37 ----
  filtered_p37 <- reactive({
    filter_data(data_p37, input$filter_code_p37, input$filter_type_p37)
  })
  
  output$table_p37 <- renderDT({
    datatable(filtered_p37(), options = list(pageLength = 10))
  })
  ...
  output$plot_p37 <- renderPlotly({
    ...
    plot_ly(df, x = ~Type, y = ~Q1, type = 'bar', name = 'Q1') %>%
      ...
      layout(barmode = 'group', title = paste("P37 -", paste(input$filter_code_p37, collapse = ", ")))
  })
  
  output$download_p37 <- downloadHandler(
    filename = function() { paste0("P37_filtered.xlsx") },
    content = function(file) { write.xlsx(filtered_p37(), file) }
  )
  
  # ---- P38 ----
  filtered_p38 <- reactive({
    filter_data(data_p38, input$filter_code_p38, input$filter_type_p38)
  })
  
  output$table_p38 <- renderDT({
    datatable(filtered_p38(), options = list(pageLength = 10))
  })
  ...
  output$plot_p38 <- renderPlotly({
    ...
    plot_ly(df, x = ~Type, y = ~Q1, type = 'bar', name = 'Q1') %>%
      add_trace(y = ~Med, name = 'Med') %>%
      add_trace(y = ~Q3, name = 'Q3') %>%
      layout(barmode = 'group', title = paste("P38 -", paste(input$filter_code_p38, collapse = ", ")))
  })
  
  output$download_p38 <- downloadHandler(
    filename = function() { paste0("P38_filtered.xlsx") },
    content = function(file) { write.xlsx(filtered_p38(), file) }
  )
  
  # ---- P39 ----
  filtered_p39 <- reactive({
    filter_data(data_p39, input$filter_code_p39, input$filter_type_p39)
  })
  
  output$table_p39 <- renderDT({
    datatable(filtered_p39(), options = list(pageLength = 10))
  })
  ...
  output$plot_p39 <- renderPlotly({
    ...
    plot_ly(df, x = ~Type, y = ~Q1, type = 'bar', name = 'Q1') %>%
      add_trace(y = ~Med, name = 'Med') %>%
      add_trace(y = ~Q3, name = 'Q3') %>%
      layout(barmode = 'group', title = paste("P39 -", paste(input$filter_code_p39, collapse = ", ")))
  })
  
  output$download_p39 <- downloadHandler(
    filename = function() { paste0("P39_filtered.xlsx") },
    content = function(file) { write.xlsx(filtered_p39(), file) }
  )
}
