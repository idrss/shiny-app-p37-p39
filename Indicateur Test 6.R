...
ui <- bs4DashPage(
  ...
  header = bs4DashNavbar(title = "Analyse Structure Financière"),
  ...
  sidebar = bs4DashSidebar(
    ...
    bs4SidebarMenu(
      ...
      bs4SidebarMenuItem("Tableau et Graphiques", tabName = "tableaux", icon = icon("chart-bar")),
  ...
  body = bs4DashBody(
    ...
    bs4TabItems(
      ...
      bs4TabItem(tabName = "tableaux",
                 fluidRow(
                   bs4Card(width = 3, title = "Filtres et Export",
                           pickerInput("filter_libelle", "Indicateur:", choices = NULL, multiple = TRUE, options = list(`actions-box`=TRUE)),
                           pickerInput("filter_famille", "Famille:", choices = NULL, multiple = TRUE, options = list(`actions-box`=TRUE)),
                   ...
                   bs4Card(width = 9,
                           tabBox(
                             ...
                             width = 12,
                             id = "graph_tabs",
                             tabPanel("Barres", plotlyOutput("plot_bar", height = "500px")),
                             tabPanel("Boxplot", plotlyOutput("plot_box", height = "500px")),
                             tabPanel("Lignes", plotlyOutput("plot_line", height = "500px"))
                           ),
                           br(),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$login_btn, {
    ...
    if(nrow(user_row) == 1) {
      ...
      updateTabItems(session, "sidebar", "tableaux")
    } else {
  ...
  output$tableau_complet <- renderDT({
    datatable(filtered_tableau(), options = list(pageLength = 15))
  })
  ...
  output$plot_bar <- renderPlotly({
    ...
    plot_ly(df, x = ~Famille, y = ~Q1, type = 'bar', name = 'Q1', color = ~Famille, colors = famille_colors) %>%
      ...
      layout(barmode = 'group', title = paste("Barres:", paste(input$filter_libelle, collapse = ", ")))
  })
  
  # Boxplot
  output$plot_box <- renderPlotly({
    df <- filtered_tableau()
    plot_ly(df, x = ~Famille, y = ~Q1, type = 'box', name = 'Q1', boxpoints = "all", color = ~Famille, colors = famille_colors) %>%
      add_trace(y = ~Med, name = 'Med', type = 'scatter', mode = 'markers+lines') %>%
      add_trace(y = ~Q3, name = 'Q3', type = 'scatter', mode = 'markers+lines') %>%
      layout(title = paste("Boxplot:", paste(input$filter_libelle, collapse = ", ")))
  })
  
  # Lignes / points
  output$plot_line <- renderPlotly({
    df <- filtered_tableau()
    df_long <- df %>% pivot_longer(cols = c(Q1, Med, Q3), names_to = "Stat", values_to = "Valeur")
    plot_ly(df_long, x = ~Famille, y = ~Valeur, type = 'scatter', mode = 'lines+markers', color = ~Stat,
            colors = c("Q1"="orange","Med"="blue","Q3"="red")) %>%
      layout(title = paste("Lignes:", paste(input$filter_libelle, collapse = ", ")))
  })
  
  # --- Télécharger ---
  output$download_table <- downloadHandler(
    filename = "Tableau_A1_D34.xlsx",
