  ...
  body = bs4DashBody(
    ...
    bs4TabItems(
      ...
      bs4TabItem(tabName = "p37",
                 fluidRow(
                   bs4Card(width = 3,
                           pickerInput("filter_code_p37", "Code:", choices = unique(data_p37$Code), multiple = TRUE, options = list(`actions-box`=TRUE)),
                           pickerInput("filter_type_p37", "Type:", choices = unique(data_p37$Type), multiple = TRUE, options = list(`actions-box`=TRUE)),
      ...
      bs4TabItem(tabName = "p38",
                 fluidRow(
                   bs4Card(width = 3,
                           pickerInput("filter_code_p38", "Code:", choices = unique(data_p38$Code), multiple = TRUE, options = list(`actions-box`=TRUE)),
                           pickerInput("filter_type_p38", "Type:", choices = unique(data_p38$Type), multiple = TRUE, options = list(`actions-box`=TRUE)),
      ...
      bs4TabItem(tabName = "p39",
                 fluidRow(
                   bs4Card(width = 3,
                           pickerInput("filter_code_p39", "Code:", choices = unique(data_p39$Code), multiple = TRUE, options = list(`actions-box`=TRUE)),
                           pickerInput("filter_type_p39", "Type:", choices = unique(data_p39$Type), multiple = TRUE, options = list(`actions-box`=TRUE)),
...
server <- function(input, output, session) {
  ...
  output$plot_p37 <- renderPlotly({
    df <- filtered_p37()
    plot_ly(df, x = ~Type, y = ~Q1, type = 'bar', name = 'Q1', color = ~Code,
            customdata = df[, c("Code", "Med", "Q3")],
            hovertemplate = "Code: %{customdata[0]}<br>Type: %{x}<br>Q1: %{y}<br>Med: %{customdata[1]}<br>Q3: %{customdata[2]}<extra></extra>") %>%
      add_trace(y = ~Med, name = 'Med') %>%
      add_trace(y = ~Q3, name = 'Q3') %>%
      layout(barmode = 'group', title = "Indicateurs P37")
  })
  output$download_p37 <- downloadHandler(filename = function(){ paste0("P37_", Sys.Date(), ".xlsx") }, content = function(file){ write.xlsx(filtered_p37(), file) })
  ...
  output$plot_p38 <- renderPlotly({
    ...
    plot_ly(df, x = ~Type, y = ~Q1, type = 'bar', name = 'Q1', color = ~Code,
      ...
      add_trace(y = ~Med, name = 'Med') %>%
      add_trace(y = ~Q3, name = 'Q3') %>%
      layout(barmode = 'group', title = "Indicateurs P38")
  })
  output$download_p38 <- downloadHandler(filename = function(){ paste0("P38_", Sys.Date(), ".xlsx") }, content = function(file){ write.xlsx(filtered_p38(), file) })
  ...
  output$plot_p39 <- renderPlotly({
    ...
    plot_ly(df, x = ~Type, y = ~Q1, type = 'bar', name = 'Q1', color = ~Code,
      ...
      add_trace(y = ~Med, name = 'Med') %>%
      add_trace(y = ~Q3, name = 'Q3') %>%
      layout(barmode = 'group', title = "Indicateurs P39")
  })
  output$download_p39 <- downloadHandler(filename = function(){ paste0("P39_", Sys.Date(), ".xlsx") }, content = function(file){ write.xlsx(filtered_p39(), file) })
}
